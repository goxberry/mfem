...
DEBUG_FLAGS = -g -O0 $(XCOMPILER)-Wall $(BASE_FLAGS)
...
# PREFIX = $(HOME)/mfem
# PREFIX = ./mfem
BUILD_DIR = $(HOME)/repos/mfem/build/
...
ZLIB_RPATH = -Wl,-rpath,$(ZLIB_DIR)/lib -L$(ZLIB_DIR)/lib

LIBUNWIND_OPT = -g
LIBUNWIND_LIB = $(if $(NOTMAC),-lunwind -ldl,)

# HYPRE library configuration (needed to build the parallel version)
HYPRE_DIR = /usr/local/opt/hypre
...
ifeq ($(MFEM_USE_SUPERLU)$(MFEM_USE_STRUMPACK),NONO)
   ifeq ($(MFEM_USE_METIS_5),NO)
     ...
     METIS_OPT =
     METIS_LIB = -Wl,-rpath,$(METIS_DIR) -L$(METIS_DIR) -lmetis
   else
     METIS_DIR = /usr/local/opt/metis
...
LAPACK_DIR = /usr/local/opt/openblas
LAPACK_OPT = -I$(LAPACK_DIR)/include
LAPACK_LIB = -Wl,-rpath,$(LAPACK_DIR)/lib -L$(LAPACK_DIR) -llapack -lblas
...
SUNDIALS_DIR = /usr/local/opt/sundials
...
SUITESPARSE_DIR = /usr/local/opt/suite-sparse
...
NETCDF_DIR = $(HOME)/local/opt/netcdf
HDF5_DIR   = $(HOME)/local/opt/hdf5
